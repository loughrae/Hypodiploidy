...
lh_v_poly_mus <- mu_lh %>%
  ...
  scale_colour_manual(values = c(`Both` = "darkgreen", `Poly only` = "darkblue", `n.s.` = "lightgrey"))
...
hypo_v_ragnum <- hyp %>% group_by(proj) %>% 
  summarize(wgd_rate = sum(wgd == 'WGD')/n(), hypo_rate = mean(group == 'Low-Hypodiploid'), avg_hyp_nohypos = median(Ragnum_hypoxia_score_pan_cancer[group != 'Low-Hypodiploid']), avg_hyp_nowgd = median(Ragnum_hypoxia_score_pan_cancer[wgd == 'No WGD'])) %>%
...
hyp_ploidy <- hyp %>% ggplot(aes(x = ploidy, y = Ragnum_hypoxia_score_pan_cancer)) +
